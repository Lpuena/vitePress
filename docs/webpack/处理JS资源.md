...
```
2. 引入eslint来配置webpack文件
- `webpack.config.js`
```js
const path = require("path");
const ESLintWebpackPlugin = require("eslint-webpack-plugin"); // [!code ++]
...
module.exports = {
  ...
  plugins: [
    new ESLintWebpackPlugin({ // [!code ++]
      // 指定检查文件的根目录 // [!code ++]
      context: path.resolve(__dirname, "src"), // [!code ++]
    }), // [!code ++]
  ],
  mode: "development",
};
...
module.exports = {
  ...
  module: {
    rules: [
      ...
      { // [!code ++]
        test: /\.js$/, // [!code ++]
        exclude: /node_modules/, // 排除node_modules代码不编译 // [!code ++]
